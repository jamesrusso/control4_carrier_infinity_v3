...
if (TEMPLATE_VERSION ~= nil) then
	TEMPLATE_VERSION.c4_device_connection_base = "2014.10.31"
end
...
DEFAULT_COMMAND_RESPONSE_INTERVAL = 6           -- If we haven't received and ACK after this many seconds, try again
...
function DeviceConnectionBase:StartCommandTimer(...)
	...
	if (self._ExpectAck) then
		...
		LogTrace(string.format("Starting Send Timer:  %d", self._SendTimer._timerID) .. " for " .. command_name)
	end
end

-- Note the '.' instead of the ':'
function DeviceConnectionBase.OnSendTimeExpired(Instance)
	LogTrace("Send Timer expired 2")
	Instance._SendTimer:KillTimer()

	local tCommand = self._LastCommand
	if (tCommand.command_name ~= nil) then
		LogTrace("Send Timer expired - Pop Command: %s, Send Next Command", tostring(tCommand.command_name))
	elseif (type(tCommand) == "string") then
		LogTrace("Send Timer expired - Pop Command: %s, Send Next Command", tostring(tCommand))
	end
